...
registerinputs.forEach(function (i) {
	const wdg = JSON.parse(JSON.stringify(registerinputbase));
	wdg.selector = "#" + i.id;
...
const seqproductionnavbaritems = {
	navbar: {
		type: "container",
		selector: "#navbarcontainer",
		options: {
...
const productionreporswidgetsall = {
	container: {
		type: "container",
		selector: "#maincontainer",
		options: {
...
const productionreportwidgets = { //Report-Page
	reporttable: {
		type: "table",
...
const productionreportwidgetsp = {
	reporttable: {
		type: "table",
		selector: "#reporttable",
		options: {
...
const tableFuncs = {
	...
	/*
	a260format: (function (data, type, row, meta) {
		if(data != null && typeof(data) == "string") {
			...
			$("#" + meta.settings.sTableId).DataTable().cell(meta.row, meta.col).data(data);
		}
		return data;
	}).toString(),
	nulldatafix: (function (data, type, row, meta) {
		if(data === "null") {
			data = "";
			$("#" + meta.settings.sTableId).DataTable().cell(meta.row, meta.col).data(data);
		}
	...
	fileorder: (function (data, type, row, meta) {
		...
		const elm = $('<label class="mini basic ui button" for="excelupload' + row.userid + '"><i class="ui upload icon"></i><span class="noclass">Excel Yükle</span><input type="file" style="display: none" id="excelupload' + row.userid + '"></label>');
			...
			uploadfunc: function (inp) {
				...
				if (inp[0][0].includes("PRİMER") == true || inp[0][0].includes("PRIMER") == true) {
					...
					for (let ii = 14; ii < 1014; ii++) {
						...
						const fmod = $.getWidget("fmodholder").query({ "val": inp[ii][1] });
						...
						const tmod = $.getWidget("tmodholder").query({ "val": inp[ii][3] });
						if (tmod.length) tempobj.tmod = tmod[0]["id"]; else tempobj.tmod = 0;

						const sca = $.getWidget("scaholder").query({ "val": inp[ii][5] });
						...
						const puri = $.getWidget("puriholder").query({ "val": inp[ii][6] });
					...
					const pairs = $.getWidget("pairholder");
					...
					for (let ii = 14; ii < 1014; ii++) {
						...
						const fmod = $.getWidget("fmodholder2").query({ "val": inp[ii][1] });
						if (fmod.length) tempobj.fmod = fmod[0]["id"]; else tempobj.fmod = 0;

						const seqn = inp[ii][2];
						tempobj.sequence = seqn.toUpperCase().replace(/\n/g, '').replace(/ /g,'');

						const tmod = $.getWidget("tmodholder2").query({ "val": inp[ii][3] });
						if (tmod.length) tempobj.tmod = tmod[0]["id"]; else tempobj.tmod = 0;
	...
	inozinbtn: (function (data, type, row, meta) {
		if (row.sequence.includes("I")) {
			const elm = $("<div class= 'ui mini blue button'><i class='icon window restore outline'></i></div>").click(function () {
	...
	apprbtn: (function (data, type, row, meta) {
		...
		const colattr = $("#" + meta.settings.sTableId).data("widget").options.datatable.columns[meta.col];
		const elm = $("<i class='large " + (data ? " green check " : " hourglass outline  ") + "circle icon'>").click(function () {
			$("#" + meta.settings.sTableId).DataTable().editor().edit($(this).closest('tr'), false).set(colattr.data, function () {
				const d = $("#" + meta.settings.sTableId).DataTable().editor().get(colattr.data);
		...
		const elm2 = $("<i class='large trash icon'>").click(function () { // TO DO <-- check breakability soln: may want to refresh fater delete
		...
		const elm3 = $("<i class='ui large " + envelopecolor + " envelope outline circle icon'>");
		elm3.click(function () {
			$.getWidget('mailmodal').show();
			const email = row.user.mail;
			const name = row.user.name;
			const date = moment(row.date).format('YYYY-MM-DD HH:mm');
			const greeting = "Sayın " + name;
			//const content = "\'" + row.ordernum + "\' nolu siparişiniz alınmıştır." 
			//+ "\nPazartesi günü senteze alınıp, Salı günü saat 16:00 itibarıyla hazır olacaktır." ;
			const content = "Siparişleriniz alınmıştır. Pazartesi günü senteze alınıp, Salı günü saat 16:00 itibarıyla hazır olacaktır.";
			...
			$("#mailenbtn").click( function() {
				const content = "Your order \'" + row.ordernum + "\' is received." 
			...
			$("#sendbutton").click(function () {
				const toSend = { action: "productionMail" };
				toSend["lang"] = "tr";
				toSend["tbl"] = row.type;
	...
	genebtn: (function (data, type, row, meta) {
		...
		const elm = $('<label class="mini basic ui button"><i class="ui check icon"></i><span class="noclass">Tamamla</span></label>');
	...
	mailbtn: (function (data, type, row, meta) {
		const colattr = $("#" + meta.settings.sTableId).data("widget").options.datatable.columns[meta.col];
		const elm = $("<i class=' envelope outline circle icon'>").click(function () {
			$.getWidget('mailmodal').show();
			const email = row.user.mail;
			const name = row.user.name;
			const content = "Sayın " + name + "\n\n'" + row.ordernum + "' nolu siparişiniz onaylanmıştır.";
	...
	link: (function (data, type, row, meta) { //set #reportbutton to show correct report
		const link = $(document.createElement("a")).addClass("tablecontent").css('cursor', 'pointer').css("text-decoration", "none").html(data).click(function (e) {
			if (row.type == undefined) {
				const arr = []; arr.push(row.id);
	...
	podlink: (function (data, type, row, meta) {
		const elm = $(document.createElement("a")).addClass("item").css('cursor', 'pointer').html(data);
		elm.click(function () {
			$.cookie("tempvar", JSON.stringify(row));
	...
	ellipsis: (function (data, type, row, meta) {
		...
		const elm = $(document.createElement("div")).addClass("ellipsis").attr("data-content", spliced).attr("data-variation", "wide").css("background-color", (data.includes("I") ? "#f2dede" : "ffffff")).html(shortened).hover(function (e) {
			$(e.target).popup("show");
		}).popup();
		$($("#" + meta.settings.sTableId).DataTable().cell(meta.row, meta.col).node()).empty().append(elm);
		return null;
	...
	ellipsis2: (function (data, type, row, meta) {
		...
		shortened += "&#8230";
		const elm = $(document.createElement("div")).html(shortened);
		$($("#" + meta.settings.sTableId).DataTable().cell(meta.row, meta.col).node()).empty().append(elm);
		return null;
	}).toString(),
	ellipsis3: (function (data, type, row, meta) {
		...
		const elmfull = $(document.createElement("div")).addClass("transition hidden").css("background-color", (data.includes("I") ? "#f2dede" : "ffffff")).css("margin-top", "10px").html(spliced);
		const elm = $(document.createElement("div")).html(shortened).css("background-color", (data.includes("I") ? "#f2dede" : "ffffff")).css("color", "#337ab7").on("click", function (e) {
	...
	redoicon: (function (data, type, row, meta) {
		...
		elm = $('<i class="redo link icon"></i>').on("click", function () {
			const self = $.getWidget('reporttable').table.DataTable().row([meta.row]);
			const selindexes = $.getWidget("reporttable").datatable.rows({ selected: true }).indexes().toArray()
	...
	navigatesynth: (function (data, type, row, meta) {
		const link = $(document.createElement("a")).addClass("tablecontent").css('cursor', 'pointer').html(data).click(function (e) {
	...
	navigateorder: (function (data, type, row, meta) {
		if (row.orderid !== 0) {
			const link = $(document.createElement("a")).addClass("tablecontent").css('cursor', 'pointer').html(data).click(function (e) {
				if (window.location.pathname === "/productionSynthesis.html") $.cookie("tempvar", JSON.stringify(row));
				else if (window.location.pathname === "/productionSynthesisProbe.html") $.cookie("tempvar", JSON.stringify(row));
				else if (window.location.pathname === "/productionGene.html") $.cookie("tempvar", JSON.stringify(row));
				window.location.href = "productionOrderDetails.html";
			});
			$($("#" + meta.settings.sTableId).DataTable().cell(meta.row, meta.col).node()).empty().append(link);
		}
	...
	linkcustomer: (function(data, type, row, meta) {
		const link = $(document.createElement("a")).addClass("tablecontent").css('cursor', 'pointer').css("text-decoration", "none").html(row.company + " - " + row.name).click(function (e) {
			const arr = []; arr.push(row.id);
			$.getWidget("modal").show(arr);
		});
		$($("#" + meta.settings.sTableId).DataTable().cell(meta.row, meta.col).node()).empty().append(link);
...
const seqproductionnavbarservices = {
	userdropdownservice: {
		source: "static",
		data: [
			{
...
const productionnavbarservices = {
...
const productionFuncs = {
	boxprint: (function (data) {

		const table_data_w_k = $.getWidget('reporttable')
			.getCols([
			...
			const order = $.getWidget('reporttable').table.DataTable().rows({ order: 'applied' } )[0];
			const table_data = [];
			...
			const toprint = table_data.reduce(function (h, obj) {
		...
		$.each(toprintreborn, function (i, val) {
			const div = $('<div>').addClass('singlereport');
			const span = $('<span>').addClass('rhead').html(val[0]['user.company']);
			const div2 = $('<div>').attr('style', "margin: 0 !important; border-top: 1px solid;");

			const div3 = $('<div>').attr('style', "margin-top: 0.1em;");

			$.each(val, function (j, v) {
				const span = $('<span>').addClass('itemLine').html(v['order.ordernum']);
				const span2 = $('<span>').addClass('primerName').html(v['name']);
		...
		const my_window = window.open('', 'Dokuman', 'status=1,width=1000,height=800');

		my_window.document.open();
		my_window.document.write('<html><head><link rel="stylesheet" href="./css/print-box.css"><script src="./jquery/jquery-3.3.1.js"></script></head>' +
	...
	boxprintz: (function (data) {
		...
		const divs = $('<div>');
		$.each(toprintreborn, function (i, val) {
			const div = $('<div>').addClass('singlereport kutuPage');
			//const span = $('<span>').addClass('rhead').html(val[0]['user.company']);
			const span22 = $('<span>').addClass('rhead').html(val[0]['order.ordernum']);
			const div2 = $('<div>').addClass('rhead_border');

			const div3 = $('<div>').attr('style', "margin-top: 0.1em;");

			$.each(val, function (j, v) {
				const span = $('<span>').addClass('itemLine').html(v['synthname']);
				const span2 = $('<span>').addClass('primerName').html(v['name']);
	...
	tubeprint: (function (data) {
		const toprint = $.getWidget('reporttable')
			.getCols([
				{ name: "user.company", render: false },
				{ name: "name", render: false },
		...
		$.each(order, function (i, v) {
			...
			const div = $('<div>').addClass('singlereport');
			const innerDiv = $('<div>').addClass('reportcontent');
			const div2 = $('<span>').addClass('compname').html(val['synthname'] + ' ' + val['sequence'].length + 'bp');
			const span = $('<span>').addClass('reportname').html(val['name'].replace(/\r?\n|\r/g,''));
			const span2 = $('<span>').addClass('reportdata').html(seq1);
			const span3 = $('<span>').addClass('reportdata2').html(seq2);
			const img = $('<img>').addClass('hideView').attr('src', './Images/logo-white.png');
			const mw = $('<div>').addClass('mw').html("MW:" + val['mw']);
			const tmbasic = $('<div>').addClass('tmbasic').html("TM:" + val['tmbasic']);
	...
	tubeprintIntergen: (function (data) {
		...
		$.each(order, function (i, v) {
			...
			const span2 = $('<span>').addClass('reportdata').html(seq1);
			const span3 = $('<span>').addClass('reportdata2').html(seq2);
			const span4 = $('<span>').addClass('nmol').html('nmol: ' + val['totalnmol']);
			const img = $('<img>').addClass('hideView').attr('src', './Images/logo-white.png');
			const mw = $('<div>').addClass('mw').html("MW:" + val['mw']);
			const tmbasic = $('<div>').addClass('tmbasic').html("TM:" + val['tmbasic']);
		...
		print6: (function(){
			const unorderedtoprint = [];

			const printtype = JSON.parse($.cookie("report"))["type"];
			if(printtype == "primer") {
				const unorderedtoprint = $.getWidget('reporttable')
				.getCols([
					{ name: "order.ordernum" },
					{ name: "sequence" },
					{ name: "dmt" },
			...
			function createPage6(el, comp, date) {
				const pageContent = $("<div></div>").addClass("page-content").css({"word-wrap":"break-word","width": "100%","max-width": "800px"}).appendTo(el);
				const outerTable = $("<table></table>").addClass("table main-table").attr("cellspacing","0").css({"border":"none"}).appendTo(pageContent);
				const outerThead = $("<thead></thead>").css({"margin":"0 !important","padding": "0 !important","border": "none"}).appendTo(outerTable);
				...
				const outerTbody = $("<tbody></tbody>").css({"border": "none"}).appendTo(outerTable);
				const mainTr = $("<tr></tr>").css({"border":"none"}).appendTo(outerTbody);
				const mainTd = $("<td></td>").addClass("single-paper").css({"padding-left": "1cm","padding-right": "1cm"}).appendTo(mainTr);
				const h4 = $("<h4></h4>").html("Sentez Raporu: <span class='itemCompanyName'>" + comp + "</span><span style='float: right'>Sentez Tarihi - " + date).appendTo(mainTd);
				const headerInput = $("<input></input>").addClass("editCompanyName hidePrint").val(comp).appendTo(mainTd);
				mainTd.append($("<hr></hr>").css({"padding-top":"0", "padding-bottom": "0", "margin-bottom": "0", "color":"#222222"}));
				const footTd = $("<td></td>").css({"padding-left": "1cm", "padding-right": "1cm"}).html("<h5>Primer Kullanım Kılavuzu:</h5> <ol style='padding-left:8px;'>"
			...
			function createSingle(el, ordering, oligoname, sequence, bp, synthno, scala, purification, gc, mw, extcoef, tmbasic, tmneighbor, tmsalt, a260, conc, ng, totalnmol, od) {
				const div = $("<div></div>").addClass("single-report").css({"border-top": "solid","border-top-width": "1px", "border-top-color": "#222222"}).appendTo(el);
				...
				const table = $("<table></table>").addClass("table").css({"margin-top": "0", "margin-bottom": "0", "border-top": "none"}).appendTo(div);
				const tbody = $("<tbody></tbody>").css({"border-top": "none"}).appendTo(table);
				const tr = $("<tr></tr>").css({"border-top": "none"}).appendTo(tbody);
				const td1 = $("<td></td>").css({"margin":"0", "padding":"0", "border-top": "none", "white-space":"nowrap"}).appendTo(tr);
				const table1 = $("<table></table>").addClass("table table-condensed").css({"border-collapse":"collapse"}).appendTo(td1);
				const tbody1 = $("<tbody></tbody>").appendTo(table1);
				...
				const td2 = $("<td></td>").css({"margin":"0", "padding":"0", "border-top": "none", "white-space":"nowrap"}).appendTo(tr);
				const table2 = $("<table></table>").addClass("table table-condensed").css({"border-collapse":"collapse"}).appendTo(td2);
				const tbody2 = $("<tbody></tbody>").appendTo(table2);
				tbody2.append($("<tr></tr>").html("<td class='col-sm-3 header'>GC</td><td class='col-sm-9'><span class='units'>%</span>" + Number(gc).toFixed(1) + "</td>"));
				...
				const td3 = $("<td></td>").css({"margin":"0", "padding":"0", "border-top": "none", "white-space":"nowrap"}).appendTo(tr);
				const table3 = $("<table></table>").addClass("table table-condensed").css({"border-collapse":"collapse"}).appendTo(td3);
				const tbody3 = $("<tbody></tbody>").appendTo(table3);
				...
				//const td4 = $("<td></td>").css({"margin":"0", "padding":"0", "border-top": "none", "white-space":"nowrap"}).appendTo(tr);
				//const table4 = $("<table></table>").addClass("table table-condensed").css({"border-collapse":"collapse"}).appendTo(td4);
				//const tbody4 = $("<tbody></tbody>").appendTo(table4);
				...
				const td5 = $("<td></td>").css({"margin":"0", "padding":"0", "border-top": "none", "white-space":"nowrap"}).appendTo(tr);
				const table5 = $("<table></table>").addClass("table table-condensed").css({"border-collapse":"collapse"}).appendTo(td5);
				const tbody5 = $("<tbody></tbody>").appendTo(table5);
			...
			const body = $("<body></body>").append($("<input/>").attr("id","tarihInput")).append($("<button></button>").attr("id","tarihDegistir").html("Toplu tarih degistir"))
			...
			const printarea = $("<div></div>").addClass("printablearea").appendTo(body);
			
			$.each(group6, function(k,v) {
				$.each(v, function(i,vv) {
					const thedate = vv[0]["synthname"].split("-")[0];
					const thedatewithdots = thedate.charAt(4) + thedate.charAt(5) + "." + thedate.charAt(2) + thedate.charAt(3) + ".20" + thedate.charAt(0) + thedate.charAt(1);
					console.log(vv[0], "   ", vv[0]["order.date"], "   ", thedate);
					const page6 = createPage6(printarea,k,thedatewithdots);
					$.each(vv, function(ii,vvv) {
						const gcperc= ((vvv.sequence.match(/G/g) || []).length + (vvv.sequence.match(/C/g) || []).length)/vvv.sequence.length*100;
						createSingle(page6, ii + 1, vvv.name, vvv.sequence, vvv.bp, vvv.synthname, vvv["primer_sca.id"], vvv["primer_puri.id"], gcperc, vvv.mw, vvv.excoef, vvv.tmbasic, "", "", vvv.a260, vvv.conc, vvv.totalng, vvv.totalnmol, vvv.od);
					});
				});
			});
			...
			my_window.document.open();
			my_window.document.write('<!DOCTYPE html><html>' +
				'<head><title>Sentez Musteri Raporu</title>' +
				'<link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css" rel="stylesheet">'
				+ '<link href="./css/contentblah.css" rel="stylesheet"/><link href="./css/report6.css" rel="stylesheet"/>'
				+'<script src="./js/modernizr.js"></script><script src="./jquery/jquery-3.3.1.js"></script></head>'
	...
	tubeprintzIntergen: (function (data) {
		...
		$.each(order, function (i, v) {
			...
			if(val['user.company'].toUpperCase().includes("NTERGEN") === false) {
			...
			const div = $('<div>').addClass('tubePage singlereport');
			const innerDiv = $('<div>').addClass('reportcontent');
			const div2 = $('<span>').addClass('compname').html(val['synthname'] + ' ' + val['sequence'].length + 'bp');
			const span = $('<span>').addClass('reportname').html(val['name'].replace(/\r?\n|\r/g,''));
			const span2 = $('<span>').addClass('reportdata').html(seq1);
			const span3 = $('<span>').addClass('reportdata2').html(seq2);
			const span4 = $('<span>').addClass('nmol').html('nmol: ' + val['totalnmol']);
			const img = $('<img>').attr('src', './Images/logo-white.png');
			const mw = $('<div>').addClass('mw').html("MW:" + val['mw']);
			const tmbasic = $('<div>').addClass('tmbasic').html("TM:" + val['tmbasic']);
	...
	tubeprintz: (function (data) {
		...
		$.each(order, function (i, v) {
			...
			const div = $('<div>').addClass('tubePage singlereport');
			const innerDiv = $('<div>').addClass('reportcontent');
			const div2 = $('<div>').addClass('compname').html(val['synthname'] + ' ' + val['sequence'].length + 'bp');
			const span = $('<span>').addClass('reportname').html(val['name'].replace(/\r?\n|\r/g,''));
			const span2 = $('<span>').addClass('reportdata').html(seq1);
			const span3 = $('<span>').addClass('reportdata2').html(seq2);
			const img = $('<img>').attr('src', './Images/logo-white.png');
...
const productionorderdetailwidgets = {
	container: {
		type: "container",
		selector: "#maincontainer",
		options: {
...
const nextsyntheseswidgetsprobe = {
	synthlisttable: {
		type: "table",
		selector: "#synthlisttable",
		options: {
...
const productionsyntheseswidgets = {
	synthesistable: {
		type: "table",
...
const productionsyntheseswidgetsp = {
	synthesistable: {
		type: "table",
		selector: "#synthesistable",
		options: {
	...
	orderlisttable: {
		...
		options: {
			...
			fetchparams: {
				...
				order: ['orders.date DESC'], //['orders.type ASC','orders.id DESC'] backend: attr.order = [db.sequelize.literal('orders.type ASC'), db.sequelize.literal('orders.id DESC')]
			},
			oncreate: true,
			table: "ui celled table",
			datatable: {
...
const seqproductionOrderListItems = {
	orderlisttable: {
		type: "table",
		selector: "#orderlisttable",
		options: {
...
adminregisterinputs.forEach(function (i) {
	const wdg = JSON.parse(JSON.stringify(adminregisterinputbase));
	wdg.selector = "#" + i.id;
	if (i.type) wdg.options.type = i.type;
	wdg.options.placeholder = i.placeholder;
	wdg.options.name = i.id;
